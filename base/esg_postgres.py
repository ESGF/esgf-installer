"""Install Postgres and configuration needed for ESGF."""
import os
import pwd
import shutil
import re
...
def check_existing_pg_version(psql_path):
    """Get the version number if a previous Postgres installation is detected."""
...
def create_pg_super_user(psycopg2_cursor, db_user_password):
    ...
    print "Create {db_user} user: ".format(db_user=config["postgress_user"]),
    psycopg2_cursor.mogrify("CREATE USER {db_user} with"
                            "CREATEROLE superuser PASSWORD \'{db_user_password}\';".format(
...
def create_pg_publisher_user(cursor, db_user_password):
    """Create postgres user for the ESGF Publisher (esgcet by default)."""
    ...
    print "Create {publisher_db_user} user:".format(
          publisher_db_user=publisher_db_user), cursor.mogrify("CREATE USER {publisher_db_user} PASSWORD \'{db_user_password}\';".format(
...
def build_connection_string(user, db_name=None, host=None, password=None):
    """Create the db connection string using the params as options."""
...
def create_pg_pass_file():
    """Create the file to store login passwords for psql."""
...
def setup_postgres_conf_file():
    """Copy postgres.conf file to proper location."""
...
def postgres_list_db_schemas(conn=None, user_name="postgres", db_name="postgres", password=None):
    """Print a list of all schemas known to postgres."""
    if not conn:
...
def postgres_list_dbs(conn=None, user_name="postgres", db_name="postgres", password=None):
    """Print a list of all databases known to postgres."""
    if not conn:
        conn = connect_to_db(user_name, db_name, password=password)
    cur = conn.cursor()
    try:
...
def postgres_clean_schema_migration(repository_id):
    """Remove entries from the esgf_migrate_version table if any exist where repository_id matches an SQL LIKE to the first argument.
