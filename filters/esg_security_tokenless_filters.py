...
def edit_web_xml(esg_filter_entry_file, dest_dir, esg_filter_entry_pattern):
    """Install esg filter into web application's web.xml file.

    Replaces a place holder token with the contents of the filter snippet file "esg-security-filter.xml".
    """
...
def install_security_tokenless_filters(dest_dir="/usr/local/tomcat/webapps/thredds"):
    """Install the security tokenless filter into a web.xml file."""
...
def initialize_esgf_mirror_jar_list():
    """Create list of jars to be downloaded from the mirror."""
    # TODO: split spring/las jars into separate function
    # ------------------------------------------------------------------
    # NOTE: Make sure that this version matches the version that is in the esg-orp project!!!
...
def get_orp_libs(service_name="thredds"):
    """Copy the filter jar file to the web app's lib dir.
    ...
    if os.path.exists(dest_dir):
        print "Copying libary jars from the ORP to {}".format(dest_dir)
        ...
        print "Downloading library jars from ESGF Distribution Server (ORP/Security) to {} ...".format(dest_dir)
