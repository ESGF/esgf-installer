...
def clone_apache_repo(devel):
    ...
    if os.path.isdir(os.path.join("apache_frontend", ".git")):
        ...
        apache_frontend_repo_local = git.Repo(
            "/usr/local/src/esgf/workbench/esg/apache_frontend/apache_frontend")
        if devel == 1:
...
def stop_httpd():
    ...
    stop_httpd_process = esg_functions.call_subprocess(
...
def start_httpd():
    ...
    start_httpd_process = esg_functions.call_subprocess(
...
def setup_apache_frontend(devel=False):
    ...
    ******************************* \n'''
    ...
    try:
        ...
        check_config_process = subprocess.Popen(
            shlex.split(check_config_command))
        ...
        while True:
            ...
            add_more_ips = raw_input(
                "Do you wish to allow more ip addresses to access admin restricted pages? y/n:")
            if add_more_ips.lower() != "y":
                break

        allowed_ip_address_string = "".join(
            "Allow from " + address + "\t" for address in ip_addresses)
        logger.debug("allowed_ip_address_string: %s",
                     allowed_ip_address_string)
        ...
        # Write the contents of /etc/tempcerts/cacert.pem  to
        ...
        if os.path.isfile(config_file):
            ...
            esgf_httpd_version_process = subprocess.Popen(
                shlex.split(esgf_httpd_version_command))
            ...
            if not esgf_httpd_version_stdout:
                ...
                if esg_version_manager.check_version_atleast(esgf_httpd_version_stdout, config.config_dictionary["apache_frontend_version"]) == 0:
                    ...
                    logger.info(
                        "esgf-httpd version is out-of-date, attempting to update.")
                    update_apache_conf(devel)
        else:
            logger.info(
                "esgf-httpd.conf file not found, attempting to update. This condition is not expected to occur and should be reported to ESGF support")
            update_apache_conf()
        ...
        with esg_bash2py.pushd("apache_frontend/apache-frontend"):
            ...
            if os.path.isfile(config_file):
                ...
                if os.path.isfile(config_file_backup_name):
                    logger.info(
                        "WARNING:  esgf-httpd.conf already backed up today.")
                    shutil.copyfile(config_file_backup_name,
                                    config_file_backup_name + ".1")
                else:
            ...
            allowed_ips_sed_process = subprocess.Popen(shlex.split(
                "sed -n '/\#permitted-ips-start-here/,/\#permitted-ips-end-here/p' /etc/httpd/conf/esgf-httpd.conf"), stdout=subprocess.PIPE)
            allowed_ips_grep_process = subprocess.Popen(shlex.split(
                "grep Allow"), stdin=allowed_ips_sed_process.stdout, stdout=subprocess.PIPE)
            allowed_ips_sort_process = subprocess.Popen(
                shlex.split("sort -u"), stdin=allowed_ips_grep_process)
            ...
            permitted_ips_process = subprocess.Popen(
                shlex.split(permitted_ips_command))
            ...
            wsgi_path_module_sed_command = 'sed -i "s/\(.*\)LoadModule wsgi_module {wsgi_path}\(.*\)/\1LoadModule wsgi_module placeholder_so\2/" etc/httpd/conf/esgf-httpd.conf;'.format(
                wsgi_path=wsgi_path)
            wsgi_path_module_sed_process = subprocess.Popen(
                shlex.split(wsgi_path_module_sed_command))
            wsgi_path_module_sed_stdout, wsgi_path_module_sed_stderr = wsgi_path_module_sed_process.communicate()
            logger.debug("wsgi_path_module_sed_stdout: %s",
                         wsgi_path_module_sed_stdout)
            logger.debug("wsgi_path_module_sed_stderr: %s",
                         wsgi_path_module_sed_stderr)

            # TODO: Terrible names; figure out what they are representing and
            # rename
            ...
            # TODO: for now, adding full path to avoid confusion with the two
            # etc directories
            with open("/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/esgf-httpd.conf", "r") as file:
                ...
                if not "#Include /etc/httpd/conf/esgf-httpd-locals.conf" in filedata:
                    ...
                    filedata = filedata.replace(
                        "Include /etc/httpd/conf/esgf-httpd-locals.conf", "#Include /etc/httpd/conf/esgf-httpd-locals.conf")
                if not '#Include /etc/httpd/conf/esgf-httpd-local.conf' in filedata:
                    uncommented_include_httpd_local_file = True
                    filedata = filedata.replace(
                        "Include /etc/httpd/conf/esgf-httpd-local.conf", "#Include /etc/httpd/conf/esgf-httpd-local.conf")
            ...
            original_server_lines_file = open(
                "/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/original_server_lines", "w")
            with open("/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/esgf-httpd.conf", "r") as esgf_httpd_conf_file:
                for i in range(22):
                    original_server_lines_file.write(
                        esgf_httpd_conf_file.readline())

            original_server_lines_file.close()

            default_server_lines_file = open(
                "/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/default_server_lines", "w")
            with open("/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/tc/httpd/conf/esgf-httpd.conf.tmpl", "r") as esgf_httpd_conf_template:
                for i in range(22):
                    default_server_lines_file.write(
                        esgf_httpd_conf_template.readline())
            ...
            # check if esgf-httpd.conf and esgf-httpd.conf.tmpl are equivalent,
            # i.e. take the diff
            if filecmp.cmp("/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/esgf-httpd.conf", "/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/esgf-httpd.conf.tmpl"):
                # we have changes; add allowed ips, ext file selection and wsgi
                # path to latest template and apply
                logger.info(
                    "Detected changes. Will update and reapply customizations. An esg-node restart would be needed to read in the changes.")

                # write /etc/httpd/conf/esgf-httpd.conf.tmpl into
                # /etc/httpd/conf/origsrvlines
                original_server_lines_file = open(
                    "/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/original_server_lines", "a")
                ...
                shutil.move("/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/original_server_lines",
                            "/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/esgf-httpd.conf.tmpl")
                shutil.copyfile("/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/esgf-httpd.conf.tmpl",
                                "/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/esgf-httpd.conf")

                replace_wsgi_module_placeholder_command = 'sed -i "s/\(.*\)LoadModule wsgi_module placeholder_so\(.*\)/\1LoadModule wsgi_module {wsgi_path}\2/" etc/httpd/conf/esgf-httpd.conf;'.format(
                    wsgi_path=wsgi_path)
                replace_wsgi_module_placeholder_process = subprocess.Popen(
                    shlex.split(replace_wsgi_module_placeholder_command))
                ...
                insert_permitted_ips_command = 'sed -i "s/\#insert-permitted-ips-here/\#permitted-ips-start-here\n{allowed_ips}\n\t#permitted-ips-end-here/" etc/httpd/conf/esgf-httpd.conf;'.format(
                    allowed_ips=allowed_ips_stdout)
                insert_permitted_ips_process = subprocess.Popen(
                    shlex.split(insert_permitted_ips_command))
                ...
                shutil.copyfile("/etc/httpd/conf/esgf-httpd.conf",
                                "/etc/httpd/conf/esgf-httpd.conf.bak")
                shutil.copyfile(
                    "/usr/local/src/esgf/workbench/esg/apache_frontend/apache-frontend/etc/httpd/conf/esgf-httpd.conf", "/etc/httpd/conf/esgf-httpd.conf")
            else:
